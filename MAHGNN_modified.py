...
class MAHGCN(nn.Module):
    def __init__(self,  act, drop_p, layer):
        ...
        self.layer=layer
        self.net_gcn_down1 = GCN(329, 1, act, drop_p)
        self.net_pool1 = AtlasMap(329, 200, drop_p)
...
class AtlasMap(nn.Module):
    ...
    def forward(self, h):
        ...
        #h = self.drop(h)
        h = h.T
        filename = '/Users/ailarmahdizadeh/Desktop/samples/mapping_'+str(self.indim) +'to' + str(self.outdim)+ '_b.mat'
...
class AtlasMap_max(nn.Module):
    ...
    def forward(self, h):
        ...
        Map = scio.loadmat(filename)
        Map = scio.loadmat(filename)
        filename = '/Users/ailarmahdizadeh/Desktop/samples/mapping_' + str(self.indim) + 'to' + str(
            self.outdim) + '_b.mat'
        Map = scio.loadmat(filename)
        Map = Map['mapping']
        Map = torch.tensor(Map)
...
class AtlasMap_th(nn.Module):
    ...
    def forward(self, h):
        ...
        #h = self.drop(h)
        h = h.T
        filename = '/Users/ailarmahdizadeh/Desktop/samples/mapping_'+str(self.indim) +'to' + str(self.outdim)+ '.mat'
        Map = scio.loadmat(filename)
        Map = Map['mapping']
        Map[Map<self.th] =0
        Map[Map>= self.th] = 1
